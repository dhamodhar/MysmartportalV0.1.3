...
$(document).ready(function(){
   $('#user_status').prop('selectedIndex',0);
			...
			var i =0;
                ...
                i =  $(this).find('RecCount').text();             
					...
					if(contract_status == "Active")
					{
                     ...
                     // map_view_status = "<a href='<?php echo base_url()?>index.php/welcome/service_contracts_map/"+contract_number+"'  title='View Asset in Map'><img src='http://lowrysmartportal.com/assets/images/viewmap.png' style='width:33%'></a>";
					}else
					{
					  map_view_status = "<td style='width:100px;'>"+contract_status+"</td>";
  ...
  function loadmore()
  {
    ...
    $("#wait").css("display", "none");
	 ...
	 //alert("<?php echo base_url()?>index.php/welcome/all_servicecontracts/null/"+total_count);
        $.ajax({
            type: "GET",
            url: "<?php echo base_url()?>index.php/welcome/all_servicecontracts/Active/"+total_count,
            dataType: "text",
            success: function(xml){
			$('#contracts-list tbody').html("");
                $(xml).find('contracts').each(function(){
...
"language": {"infoEmpty": "No Response - Cannot process the data."},	});
  ...
  <script>
  function getdetails_by_location(id)
  {
  
  $("#service_contracts_map").attr("href", "<?php echo base_url()?>index.php/welcome/active_contracts_map/"+id);
        $.fn.dataTable.ext.errMode = 'none';
		var data = $('#contracts-list').dataTable();
		data.fnDestroy();
		$('#contracts-list tbody').html("");
      ...
      $.ajax({
            type: "GET",
            url: "<?php echo base_url()?>index.php/welcome/all_servicecontracts/Active/ /"+id,
            dataType: "text",
            success: function(xml){
                $(xml).find('contracts').each(function(){
...
function saveexcel()
{
    ...
    window.open('<?php echo base_url()?>index.php/welcome/all_servicecontracts_to_csv', '_blank');
}
</script>
    </body>
</html>